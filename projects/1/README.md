...
* branchPredictor=true : The processor has a branch predictor that can predict the direction of a branch.  100% prediction rate is assumed.  That means on a taken branch, the correct path instructions can start to be fetched after the ID stage when the target address is decoded, instead of the EX stage when the branch condition is evaluated.  Not taken branches do not need to be flushed and re-fetched so they are irrelevant.
* branchTargetBuffer=true : The processor has a branch target buffer that can predict the branch target as well as the direction.  Again 100% prediction rate is assumed.  That means on a taken branch, the correct path instruction can start to be fetched immediately after the IF stage because the target can be predicted at IF even before decoding.
...
* RAW (Read After Write) hazards: When an instruction reads a register written by a previous instruction.  Note that the previous instruction can be fetched to the IF stage in the same cycle in the case of a 2-wide processor.
...
* WAW (Write After Write) hazards: This is when two instructions attempt to write to the same register simultaneously.  This would never happen in a 1-wide processor but it can happen in a 2-wide process when a LOAD instruction and an ALU instruction both attempt to write to the same register at the WB stage.

Data hazard avoidance happens at the ID stage since that is the stage when registers are read from the register file.  If there is a data hazard, the ID stage would be cancelled and the instruction would stall at the IF stage.  Only when the data hazard is resolved is the instruction allowed to proceed to the ID stage and read registers.  So all instruction on the issue queue should be free of data hazards.
